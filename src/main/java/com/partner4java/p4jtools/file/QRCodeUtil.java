...
public class QRCodeUtil {
	...
	public static void encode(String content, int width, int height, String srcImagePath,
			String destImagePath) {
		try {
			ImageIO.write(genBarcode(content, width, height, srcImagePath), "png", new File(
					destImagePath));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static BufferedImage genBarcode(String content, int width, int height,
			String srcImagePath) throws Exception {
		...
		for (int y = 0; y < matrix.getHeight(); y++) {
			for (int x = 0; x < matrix.getWidth(); x++) {
				...
				if (x > halfW - IMAGE_HALF_WIDTH && x < halfW + IMAGE_HALF_WIDTH
					...
					pixels[y * width + x] = srcPixels[x - halfW + IMAGE_HALF_WIDTH][y - halfH
							+ IMAGE_HALF_WIDTH];
				}
				// 在图片四周形成边框
