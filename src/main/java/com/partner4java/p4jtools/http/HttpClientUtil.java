...
import com.partner4java.p4jtools.enums.PlatformEnum;
...
public class HttpClientUtil {
	 ...
	 * @return
	 */
	public static int getPlatformName(String agent) {
		agent = agent.toLowerCase();
		if (agent != null && !agent.trim().equals("")) {
			if (agent.contains("mobile")) {
				if (agent.contains("micromessenger")) {
					return PlatformEnum.WECHART.value();
				} else if (agent.contains("ios")) {
					return PlatformEnum.CLIENT_IOS.value();
				} else if (agent.contains("android")) {
					return PlatformEnum.CLIENT_ANDROID.value();
				}
			} else {
				return PlatformEnum.PC.value();
			}
		}
